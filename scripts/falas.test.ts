 ...
 * resposta da Cora, porque a transcrição da entrada chega depois que a
 * resposta já começou.
 */
...
function turnoCompleto(): Fala[] {
  ...
  // 2. a Cora já começa a responder, antes de a transcrição da entrada existir
  falas = escrever(falas, "cora", "cora:item_2", "Oi, ", true, true, 1100);
  falas = escrever(falas, "cora", "cora:item_2", "aqui é a Cora.", true, true, 1200);
...
test("delta soma e texto final substitui", () => {
  let falas = escrever([], "cora", "a", "Card", true, true, 1);
  falas = escrever(falas, "cora", "a", "ioLight", true, true, 2);
  assert.equal(falas[0].texto, "CardioLight");
  falas = escrever(falas, "cora", "a", "CardioLight.", false, false, 3);
  assert.equal(falas[0].texto, "CardioLight.");
...
test("vários turnos mantêm a ordem alternada", () => {
  ...
  for (let n = 1; n <= 3; n++) {
    ...
    falas = escrever(falas, "cora", `cora:${n}`, `resposta ${n}`, false, false, n * 100 + 10);
...
test("o texto para copiar omite o turno que nunca foi transcrito", () => {
  ...
  assert.equal(
    paraTexto(falas, "Cora"),
    "Cliente: meu Holter está apitando\n\nCora: Oi, aqui é a Cora.",
  );
});
